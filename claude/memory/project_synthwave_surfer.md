...
- **Layer A** (billig, hoher Fan-out, IN ARBEIT auf Branch `genre-layer-a`): Half-Time-Drum-Grid, `arp`-Aktivierung (Feld existiert, wird nie gelesen!), Per-Track-Distortion + Supersaw-Voice, **tempo-synchroner LFO (Wobble)**. → Techno, Prog House, Trance, Dubstep, Chillstep.
  ...
  - **WAV-EXPORT GEFIXT (v0.1.2, 2026-06-07):** Offline-Render via `setContext()`+`OfflineContext.render()`; `buildAudioGraph`/`scheduleAll` nutzen `getDestination()`/`getTransport()` statt der frozen globalen `Tone.Destination`/`Tone.Transport` (die folgen dem Context-Swap NICHT → war Cross-Context-Connect-Fehler). Alle 9 Genres exportieren. Lehre: in Offline-Render nie gegen global `Tone.Destination` verbinden.
  ...
  - **NÄCHSTES (offen, User-Reihenfolge):** Feintuning (rollende Offbeat-Bässe Techno/Prog/Trance, Dubstep Sub-Layer + per-Takt-Wobble-Automation, Detroit-Stab, Drop-Timing) — alles im Audit-JSON konkret. Dann Lo-fi/Chillstep/DnB + Tier 3 Bach. **Public-facing In-App-Text in v0.1.1 GEFIXT** (Epigraph „Three algorithms"→„Nine engines", Footer „Outrun×Noir×Dreamwave"→„Nine genre engines", Header-Badge „v1.0 · Factory Presets"→akkurate Feature-Liste; Hero-Screenshot neu). Release v0.1.1 auf beiden Plattformen. **Tote-Code-Cleanup ERLEDIGT:** FACTORY_PRESETS/applyPreset/renderFactoryRow + zugehöriges CSS entfernt (−70 Zeilen; `scripts/fullsuite.mjs` Slice D mit entfernt; Suite 45/0, Runtime-Smoke 0 Fehler). **v0.1.2 (2026-06-07) released auf beiden Plattformen:** WAV-Export gefixt (s.o.); **Genre-Wechsel während Playback** plant jetzt die Komposition neu (`generate()`→`startPlayback()` wenn Transport läuft — vorher nur Voicing live, daher „nur Sound wechselt"); **dubstep halftime** Same-time-Hat-Dedup im Scheduler (mono-Voice kann nicht doppelt zur selben Zeit triggern); + Tote-Code-Cleanup. Alle 9 Genres exportieren, Suite 45/0, Playwright-Verifikation grün. **v0.2.0 (2026-06-08) — EDM-Feintuning released:** rollende Offbeat-Bässe (Techno sustained+melodisch via stepDur/moveProb, Prog/Trance 16tel-Roll), **Techno-Detroit-Stab** statt Pad (`padStab`-Config + `buildPadHits`; gibt Frequenzraum frei), EDM-Pads leiser/dunkler/trockener, **Supersaw-Leads** Techno/Prog/Trance (per-Genre `leadRoot:57` = Oktave tiefer, sustain 0.10, Phrase über beide Mains+Breakdown wiederholt in `buildArrangedForm`), **Dubstep GEPARKT** (Card via CSS versteckt — echtes Wobble braucht dedizierten Filter-Knoten im Bass-Bus, weil der Bus-LFO den `alkali`-PolySynth-per-Voice-Filter nicht erreicht; Code bleibt), Piano-Roll zeigt jetzt Pad/Stab korrekt (geteiltes `buildPadHits` für Scheduler+Roll). **bassFill-Rest-Bug** gefixt (verlor `rest`-Flag → NaN-Noten → blanke Roll bei Rolling-Bass-Genres + still gedroppte Bassnoten). Suite 47/0. **OFFEN:** Trance „Pad/Lead nie solo im Breakdown" (+ Idee Trance-Pad→`acid`-Modell); dann Lo-fi/Chillstep/DnB (Layer B) + Tier 3 Bach; Dubstep-Wobble (dedizierter Filter-Knoten) später.
