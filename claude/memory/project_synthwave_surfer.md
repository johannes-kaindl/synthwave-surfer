...
- **Layer A** (billig, hoher Fan-out, IN ARBEIT auf Branch `genre-layer-a`): Half-Time-Drum-Grid, `arp`-Aktivierung (Feld existiert, wird nie gelesen!), Per-Track-Distortion + Supersaw-Voice, **tempo-synchroner LFO (Wobble)**. → Techno, Prog House, Trance, Dubstep, Chillstep.
  ...
  - **NÄCHSTES (offen, User-Reihenfolge):** Feintuning (rollende Offbeat-Bässe Techno/Prog/Trance, Dubstep Sub-Layer + per-Takt-Wobble-Automation, Detroit-Stab, Drop-Timing) — alles im Audit-JSON konkret. Dann Lo-fi/Chillstep/DnB + Tier 3 Bach. Tote-Code-Cleanup (FACTORY_PRESETS) + WAV-Export-Bug (parked). **Public-facing In-App-Text in v0.1.1 GEFIXT** (Epigraph „Three algorithms"→„Nine engines", Footer „Outrun×Noir×Dreamwave"→„Nine genre engines", Header-Badge „v1.0 · Factory Presets"→akkurate Feature-Liste; Hero-Screenshot neu). Release v0.1.1 auf beiden Plattformen. OFFEN: nur noch der FACTORY-Source-Kommentar „six" statt fünf (toter Code, vernachlässigbar).
