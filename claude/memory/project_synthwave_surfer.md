...
- **Layer A** (billig, hoher Fan-out, IN ARBEIT auf Branch `genre-layer-a`): Half-Time-Drum-Grid, `arp`-Aktivierung (Feld existiert, wird nie gelesen!), Per-Track-Distortion + Supersaw-Voice, **tempo-synchroner LFO (Wobble)**. → Techno, Prog House, Trance, Dubstep, Chillstep.
  ...
  - **ENDSTAND 2026-06-06 (Branch `genre-layer-a`, 18 Commits vor `main`, NICHT gemerged; Suite 45/45):**
  ...
  - **NÄCHSTES (offen, User-Reihenfolge):** Feintuning (rollende Offbeat-Bässe Techno/Prog/Trance, Dubstep Sub-Layer + per-Takt-Wobble-Automation, Detroit-Stab, Drop-Timing) — alles im Audit-JSON konkret. Dann **Branch → `main` mergen**. Dann Lo-fi/Chillstep/DnB + Tier 3 Bach. Tote-Code-Cleanup (FACTORY_PRESETS) + WAV-Export-Bug (parked).
